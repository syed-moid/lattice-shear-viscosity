...
# Relaxed lattice constants (m) from dft/qe/<material>/relax/vc_relax.out
# and cell masses; densities follow from the relaxed volumes.
LATTICE = {"SrTiO3": 3.9383e-10, "BaTiO3": 4.0254e-10}
...
def acoustic_velocities(mat: str) -> list[dict]:
    ...
    for direction, (gamma_index, indices) in DIRECTIONS.items():
        ...
        for i in indices:
            ...
            acoustic = np.sort(freqs[np.argsort(np.abs(freqs))[:3]])
            q_norms.append(q_si)
...
def main() -> None:
    ...
    for mat in ("SrTiO3", "BaTiO3"):
        all_rows.extend(acoustic_velocities(mat))
    ...
    for mat, lit in LITERATURE.items():
        ...
        volume = LATTICE[mat] ** 3
        header.append(
            f"#   {mat}: rho_calc = {mass / volume:.1f} kg/m^3 (relaxed cell), "
            f"rho_lit = {lit['density']:.0f} kg/m^3; {lit['citation']}"
        )
