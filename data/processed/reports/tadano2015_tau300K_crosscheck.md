# Tadano & Tsuneyuki (2015) Fig. 7 reference values — SrTiO₃ tau(300 K) / kappa(T) cross-check
...
| omega_q band (cm⁻¹) | tau_q range (ps) | Feature |
|---|---|---|
...
1. Plot per-mode tau_q(300 K) = 1/(2 Gamma_q) from the local run on the
   inset's axes (omega 0–850 cm⁻¹, tau 0.1–100 ps, log y) and compare
...
2. Compare kappa(300 K). **Current local value 3.34 W/mK vs the paper's
   ...
   before the SrTiO₃ Gamma_anh(q,T) set is treated as validated.**
...
3. The tau comparison (step 1) localizes the discrepancy in omega: if
   the 200–450 cm⁻¹ plateau matches but the < 150 cm⁻¹ tail does not,
...
the SCPH run has NO stable modes below 50 cm⁻¹ (on the 8x8x8 mesh the
smallest nonzero-|q| acoustic frequencies are ~55 cm⁻¹ = v*q_min; the
paper's <50 cm⁻¹ tail comes from its finer 12x12x12 mesh reaching
...
the SrTiO₃ Gamma_anh dataset is LIFTED **at T=300 K** (recorded in
...
- Tadano_BTO.pdf = Masuki, Nomoto, Arita, Tadano, arXiv:2205.08789
  ...
  applied to BaTiO₃ phase transitions. NOT the pending PRB 103, 094108
  (2021) pull, but contains SCP treatment of the BaTiO₃ soft mode across
...
- Vogt (1995) Fig. 6 digitization already exists, committed, in
  ...
  omega_s(T) + damping gamma(T), headerless CSV, T[K], value[cm⁻¹];
  ...
  re-digitizing. Derived anchors there: omega0(300 K) = 89.24 cm⁻¹ =
  11.06 meV; gamma(300 K) = 23.0 cm⁻¹ full -> Gamma0 = 11.5 cm⁻¹.
